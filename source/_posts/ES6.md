...
cover: https://f005.backblazeb2.com/file/img-forWeb/uPic/PbmfQb.png
...
### 1.1 ES6 和 `JavaScript` 的区别
ES6(ECMAScript 2015)是`ECMAScript`规范的第六个版本，而`JavaScript`是基于`ECMAScript`规范实现的编程语言, ES6可以被看作是 `JavaScript` 的一个重要的版本更新。
...
- ES6引入了"函数参数默认值"。
- ES6引入了"`Promise`对象", 简化了异步编程, 使其更可读和可维护。
- ES6引入了 `Set`、`Map` 等。
...
let balance = 100
let PI = 3.14
*ES6不区分整型和浮点数，所有数字都使用`number`类型来表示* 
### 2.2 常量
...
```JavaScript
    ...
    //let web = 'blog.peiong.co'
    let web = '博客网站'
    let webArr = [...web] //使用扩展运算符将 字符串 转换为 数组
    console.log("webArr", webArr) //['学', '习', '编', '程']
...
```JavaScript
    ...
    person.set('web', "https://blog.peiong.co")
...
let person = {
    ...
    person.web = "https://blog.peiong.co"
    console.log("person", person)
...
class Person {
    ...
    let person = new Person("张三", "blog.peiong.co")
...
class Person {
    ...
    let person = new Person("张三", "https://blog.peiong.co")

    console.log("person", person)
    console.log("web", person.web) //使用存取器 getter 获取私有属性
    console.log("info", person.info())

    person.web = "https://blog.peiong.co" //使用存取器 setter 设置私有属性
    console.log("web", person.web)
```
...
```JavaScript
    ...
    let paris = new Paris("张三", "男", "https://blog.peiong.co")

    console.log("paris", paris)
    console.log("web", paris.web)
    console.log("eat", paris.eat())
    
    console.log("gender", paris.gender)
    console.log("sleep", paris.sleep())
```
...
```JavaScript
    ...
    // 判断是否包含某个字符串
    let result = web.includes("paris") // true
    console.log("是否包含某个字符串:", result)

    // 返回字符串中第一次出现某个字符串的位置,若不存在则返回-1
    let result2 = web.indexOf("paris")
    console.log("返回字符串中第一次出现某个字符串的位置:", result2)

    let result3 = "https://blog.peiong.co".indexOf("peiong")
    console.log("返回字符串中第一次出现某个字符串的位置:", result3)

    // 判断一个字符串是否以指定的前缀开头
    let result4 = "https://blog.peiong.co".startsWith("https")
    console.log("判断一个字符串是否以指定的前缀开头:", result4)

    // 判断一个字符串是否以指定的后缀结尾
    let result5 = "https://blog.peiong.co".endsWith("com")
    ...
    //重复字符串
    let repeatstr = "Paris".repeat(3)
    console.log("重复3次字符串", repeatstr)

    //在字符串前添加指定数量的填充字符, 直到该字符串达到指定的长度
    let padStart = "Paris".padStart(15, "-") //由于 David 占 5 个字符, 因此只需要再添加 10 个横线, 即可达到总长度 15
    //let padStart = "Paris".padStart(15) //默认空格
